...
class TreeRepository(Generic[Model, ID]):
    ...
    async def create_node(self, data: dict) -> Model:
        ...
        try:
            ...
            if parent_id is None:
                ...
                max_rgt = (
                    max_rgt.scalar() or 0
                )  # Default to 0 if no nodes exist
    ...
    async def _shift_nodes(self, start_rgt: int, shift: int) -> None:
        ...
        result = await self.session.execute(
            select(self.model).filter(self.model.lft >= start_rgt)
        )
    ...
    async def validate_no_cycles(self, node_id: ID, parent_id: ID) -> None:
        ...
        while current_parent_id is not None:
            ...
            current_parent = await self.session.get(
                self.model, current_parent_id
            )
